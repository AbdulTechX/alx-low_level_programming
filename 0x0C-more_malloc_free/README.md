...
Tests ✔️
    ...
    main.c :test files. Provided by ALX School.
Header File 📁

Prototype

1. void *malloc_checked(unsigned int b);
2. char *string_nconcat(char *s1, char *s2, unsigned int n);
3. char *string_nconcat(char *s1, char *s2, unsigned int n);
4. int *array_range(int min, int max);
5. void *_realloc(void *ptr, unsigned int old_size, unsigned int new_size);
